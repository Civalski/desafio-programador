...
const attachPersistedPages = async (db, job) => {
  ...
  const { results } = await db.prepare('SELECT value_json FROM transcription_pages WHERE job_id=? ORDER BY page_number').bind(job.id).all();
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (action === 'delete') { if (data.fileKey) await env.DOCUMENTS.delete(data.fileKey); await env.DB.prepare('DELETE FROM transcription_pages WHERE job_id=?').bind(data.id).run(); await env.DB.prepare('DELETE FROM transcription_jobs WHERE id=?').bind(data.id).run(); return json({ ok: true }); }
  ...
  ,async scheduled(_event, env) {
    ...
    for (const job of results) { if (job.file_key) await env.DOCUMENTS.delete(job.file_key); await env.DB.prepare('DELETE FROM transcription_pages WHERE job_id=?').bind(job.id).run(); await env.DB.prepare('DELETE FROM transcription_jobs WHERE id=?').bind(job.id).run(); }
  }
};
