...
export function SavedExtractions({ onOpen, onResume, onBack }) {
  ...
  return <section className="upload-card" style={{ maxWidth: '900px', margin: '0 auto' }}>
    ...
    {items.length === 0 ? <p style={{ color: 'var(--text-muted)' }}>Nenhuma extração salva.</p> : <div className="table-container"><table className="data-table"><thead><tr><th>Arquivo</th><th>Tipo</th><th>Status</th><th>Auditoria</th><th>Progresso</th><th>Atualizado</th><th></th></tr></thead><tbody>{items.map(item => <tr key={item.id}><td>{item.fileName || item.id}</td><td>{item.tipo === 'holerite' ? 'Holerite' : 'Cartão de ponto'}</td><td>{labels[item.status] || item.status}</td><td>{item.audit?.status === 'ok' ? 'Sem alertas' : item.audit?.status === 'review_required' ? 'Revisão necessária' : '-'}</td><td>{item.progress?.current || 0}/{item.progress?.total || 0} ({item.progress?.percentage || 0}%)</td><td>{item.updatedAt ? new Date(item.updatedAt).toLocaleString('pt-BR') : '-'}</td><td style={{ whiteSpace: 'nowrap' }}>{item.status === 'concluido' && <button className="btn-secondary" onClick={() => onOpen(item)}>Abrir</button>} {item.status !== 'concluido' && <button className="btn-success" onClick={() => onResume(item)}>Retomar</button>} <button className="btn-secondary" onClick={() => remove(item)}>Excluir</button></td></tr>)}</tbody></table></div>}
  </section>;
}
